...
function randomVarName(seed) {
  const chars = ['i', 'l', 'I'];
  const rng = mulberry32(seed * 999983 + 12345);
  const length = Math.floor(rng() * 11) + 5;
  let name = '_';
  for (let i = 0; i < length; i++) name += chars[Math.floor(rng() * 3)];
  return name;
}
function rand(n = 6) {
  const c = 'abcdefghijklmnopqrstuvwxyz';
  let s = '_';
  for (let i = 0; i < n; i++) s += c[randomInt(0, 25)];
  return s;
}
...
function mangleNames(code) {
  const LUA_GLOBALS = new Set([
    'print','string','table','math','os','io','debug','require','pcall','xpcall',
    'tostring','tonumber','type','pairs','ipairs','next','load','loadstring','error',
    'assert','select','unpack','rawget','rawset','rawlen','setmetatable','getmetatable',
    'coroutine','bit','bit32','getfenv','setfenv','collectgarbage','dofile','loadfile',
    'rawequal','warn','_ENV','_G','_VERSION','game','workspace','script','Instance',
    'Vector3','Vector2','CFrame','Color3','UDim2','UDim','Enum','task','wait',
  ]);
  const varMap = {};
  ...
  while ((match = declPattern.exec(code)) !== null) {
    const name = match[1];
    if (!LUA_GLOBALS.has(name) && !varMap[name]) varMap[name] = rand(8);
  }
  for (const [old, newName] of Object.entries(varMap))
    code = code.replace(new RegExp(`(?<![a-zA-Z0-9_])${old}(?![a-zA-Z0-9_])`, 'g'), newName);
  return code;
}
function encodeStrings(code) {
  const encode = (content, quote) => {
    try {
    ...
    } catch { return `${quote}${content}${quote}`; }
  };
  code = code.replace(/"((?:[^"\\]|\\.)*)"/g, (_, c) => encode(c, '"'));
  code = code.replace(/'((?:[^'\\]|\\.)*)'/g, (_, c) => encode(c, "'"));
  return code;
}
...
app.listen(PORT, () => console.log(`Nodet running on http://localhost:${PORT}`));
